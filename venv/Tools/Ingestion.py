...
class IngestionObject:
    ...
    def insert_fy_product_description(self, fy_catalog_number, fy_manufacturer_part_number, manufacturer_part_number, is_product_number_override, manufacturer_id, default_image_id,
                                      ...
                                      db_gsa_product_price_id, db_va_product_price_id, db_ecat_product_price_id,
                                      db_htme_product_price_id, db_intramalls_product_price_id,
                                      vendor_product_notes, vendor_is_discontinued, VendorProductName, VendorProductDescription, CountryOfOriginId, MinimumOrderQty, LeadTimeDays, IsHazardous,
        ...
        if (len(self.product_description_insert_collector) > self.load_limit):
            self.product_description_insert_collector.append((fy_catalog_number, fy_manufacturer_part_number, manufacturer_part_number, is_product_number_override, manufacturer_id, default_image_id,
                                      ...
                                      fy_category_id, fy_is_green, fy_is_latex_free, fy_cold_chain, fy_controlled_code,
                                      fy_naics_code_id, fy_unspsc_code_id, fy_special_handling_id, fy_shelf_life_months, fy_product_notes,
                                                              db_gsa_product_price_id, db_va_product_price_id,
                                                              db_ecat_product_price_id, db_htme_product_price_id,
                                                              db_intramalls_product_price_id,
                                      vendor_product_notes, vendor_is_discontinued, VendorProductName, VendorProductDescription, CountryOfOriginId, MinimumOrderQty, LeadTimeDays, IsHazardous,
                                      b_website_only, gsa_eligible, va_eligible, ecat_eligible, htme_eligible, intramalls_eligible,
                                      vendor_list_price, discount, fy_cost, estimated_freight, fy_landed_cost, markup_percent_fy_sell,
                                      fy_sell_price, markup_percent_fy_list, fy_list_price, is_discontinued, price_toggle, data_toggle, av_toggle,
    ...
    def insert_product_price(self, newFyProductNumber,newFyPartNumber,newProductTaxClass,newVendorPartNumber, newProductId,newVendorId,newUnitOfIssueSymbolId,newUnitOfMeasureSymbolId,newUnitOfIssueQuantity, newProductDescriptionId, FyProductNotes, IsDiscontinued, VendorProductName, VendorProductDescription, CountryOfOriginId, MinumumOrderQty, LeadTimeDays, IsHazardous):
        if (len(self.product_insert_collector) > self.load_limit):
            ...
            self.product_insert_collector.append((newFyProductNumber, newFyPartNumber,
                                                ...
                                                newUnitOfIssueSymbolId,newUnitOfMeasureSymbolId,newUnitOfIssueQuantity, newProductDescriptionId, FyProductNotes, IsDiscontinued, VendorProductName, VendorProductDescription, CountryOfOriginId, MinumumOrderQty, LeadTimeDays, IsHazardous))
    ...
    def update_product_price_nouoi(self, newProductPriceId, newFyProductNumber,newFyPartNumber,
                                   ...
                                   newVendorId, newProductDescriptionId, FyProductNotes, IsDiscontinued, VendorProductName, VendorProductDescription, CountryOfOriginId, MinumumOrderQty, LeadTimeDays, IsHazardous):
        if (len(self.product_update_nouoi_collector) > self.load_limit):
            self.product_update_nouoi_collector.append((newProductPriceId, newFyProductNumber,newFyPartNumber,newProductTaxClass,newVendorPartNumber,newProductId,newVendorId, newProductDescriptionId, FyProductNotes, IsDiscontinued, VendorProductName, VendorProductDescription, CountryOfOriginId, MinumumOrderQty, LeadTimeDays, IsHazardous))
            ...
            self.product_update_nouoi_collector.append((newProductPriceId, newFyProductNumber, newFyPartNumber,
                                                newProductTaxClass, newVendorPartNumber, newProductId, newVendorId,
                                                newProductDescriptionId, FyProductNotes, IsDiscontinued, VendorProductName, VendorProductDescription, CountryOfOriginId, MinumumOrderQty, LeadTimeDays, IsHazardous))
    ...
    def ecat_product_price_insert(self, newProductDescriptionId, newFyProductNumber, newOnContract,
                               newApprovedLandedCost, newApprovedSellPrice, newApprovedListPrice,
                               newContractNumber, newContractModificatactionNumber, newECATPricingApproved,
        ...
        if (len(self.product_insert_collector) > self.load_limit):
            self.product_insert_collector.append(
                (newProductDescriptionId, newFyProductNumber, newOnContract, newApprovedBasePrice, newApprovedSellPrice,
                 newApprovedListPrice, newContractNumber, newContractModificatactionNumber,
    ...
    def va_product_price_insert(self, newProductDescriptionId, newFyProductNumber, newOnContract, newApprovedBasePrice,
        ...
        if (len(self.product_insert_collector) > self.load_limit):
            self.product_insert_collector.append((newProductDescriptionId, newFyProductNumber, newOnContract, newApprovedBasePrice, newApprovedSellPrice, newApprovedListPrice, newContractNumber, newContractModificatactionNumber, newVAPricingApproved, newVAApprovedPriceDate, newApprovedPercent, newMFCPercent, newVA_SIN, newVAProductNotes))
    ...
    def va_product_price_update(self, newVAProductPriceId, newProductDescriptionId, newFyProductNumber, newOnContract,
                                newApprovedBasePrice, newApprovedSellPrice, newApprovedListPrice,
                                newContractNumber, newContractModificatactionNumber, newVAPricingApproved, newVAApprovedPriceDate,
                                newApprovedPercent, newMFCPercent, newVA_SIN, newVAProductNotes):
        if (len(self.product_update_collector) > self.load_limit):
            self.product_update_collector.append((newVAProductPriceId, newProductDescriptionId, newFyProductNumber, newOnContract, newApprovedBasePrice, newApprovedSellPrice, newApprovedListPrice, newContractNumber, newContractModificatactionNumber, newVAPricingApproved, newVAApprovedPriceDate, newApprovedPercent, newMFCPercent, newVA_SIN, newVAProductNotes))
    ...
    def set_ecat_toggles(self,base_id, fy_product_number, ecat_contract, ecat_approved, ecat_product_notes):
        if (len(self.product_ecat_toggle_collector) > self.load_limit):
            self.product_ecat_toggle_collector.append((base_id, fy_product_number, ecat_contract, ecat_approved, ecat_product_notes))
    ...
    def set_gsa_toggles(self,base_id, fy_product_number, gsa_contract, gsa_approved, gsa_product_notes):
        if (len(self.product_gsa_toggle_collector) > self.load_limit):
            self.product_gsa_toggle_collector.append((base_id, fy_product_number, gsa_contract, gsa_approved, gsa_product_notes))
    ...
    def set_htme_toggles(self,base_id, fy_product_number, htme_contract, htme_approved, htme_product_notes):
        if (len(self.product_htme_toggle_collector) > self.load_limit):
            self.product_htme_toggle_collector.append((base_id, fy_product_number, htme_contract, htme_approved, htme_product_notes))
    ...
    def set_intramalls_toggles(self,base_id, fy_product_number, intramalls_contract, intramalls_approved, intramalls_product_notes):
        if (len(self.product_intramalls_toggle_collector) > self.load_limit):
            self.product_intramalls_toggle_collector.append((base_id, fy_product_number, intramalls_contract, intramalls_approved, intramalls_product_notes))
            self.obDal.set_intramalls_toggles(self.product_intramalls_toggle_collector)
            self.product_intramalls_toggle_collector = []

        else:
            self.product_intramalls_toggle_collector.append((base_id, fy_product_number, intramalls_contract, intramalls_approved, intramalls_product_notes))

    def set_intramalls_toggles_cleanup(self):
        if self.product_intramalls_toggle_collector != []:
            self.obDal.set_intramalls_toggles(self.product_intramalls_toggle_collector)


    def set_va_toggles(self,base_id, fy_product_number, va_contract, va_approved, va_product_notes):
        if (len(self.product_va_toggle_collector) > self.load_limit):
            self.product_va_toggle_collector.append((base_id, fy_product_number, va_contract, va_approved, va_product_notes))
